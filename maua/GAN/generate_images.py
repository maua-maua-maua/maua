...
if __name__ == "__main__":
    ...
    for seed, img in zip(args.seeds, imgs):
        img = tensor2img(img)
        img.save(
            f'{args.out_dir}/{Path(args.model_file.replace("/network-snapshot", "")).stem}_seed{seed}_{args.resize_strategy}{args.resize_layer}_{img.size[0]}x{img.size[1]}.jpg'
        )
