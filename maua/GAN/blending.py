...
def soup_v1():
    checkpoint_dirs = [
        "/home/hans/modelzoo/koanGAN/embers",
    ...
    sample_strategy = "random"  # "bimodal", 'uniform'
    blend_strategy = "random"  # 'crossover'
    number = 10
    num_ckpts = 4
    out_dir = "/home/hans/modelzoo/koanGAN/"
...
def generate_images(generator, file_name, batch_size=4):
    # evaluate new checkpoint
    generator.eval()
    sample_z = torch.randn((60, 512)).cuda()
    sample = []
    for sub in range(0, len(sample_z), batch_size):
        subsample = generator(sample_z[sub : sub + batch_size])
        sample.append(subsample.detach().cpu())
    sample = torch.cat(sample).detach()
...
def blend_checkpoints(checkpoints, blend_strategy, architecture, generator):
    ...
    if blend_strategy == "crossover":
        ...
        weights = [
            get_blend_weights(
                midpoints=torch.randint(-1, generator.num_ws + 1, (1,)),
                width=torch.rand(1) * generator.num_ws / 2,
                n_layers=generator.num_ws,
            ).squeeze()
            for _ in range(len(checkpoints))
        ]
...
def modelsoup(ckpt_A, ckpt_B, blend_strategy, architecture="stylegan2", number=10, out_dir="output/"):
    generator = get_generator_class(architecture)(model_file=None, output_size=(1024, 1024)).cuda()

    for n in trange(number):
        ...
        file_name = f"{out_dir}/{name}-{blend_strategy}blend{n + 1}"

        # perform blending
        state_dict = blend_checkpoints(checkpoints, blend_strategy, architecture, generator)
        generator.load_state_dict(state_dict)
        save_dict = {
            "G_ema": {
                k.replace("mapper.G_map", "mapping").replace("synthesizer.G_synth", "synthesis"): v
        ...
        # evaluate new checkpoint
        generate_images(generator, file_name)
        torch.save(save_dict, f"{file_name}.pt")


if __name__ == "__main__":
    modelsoup(
        "/home/hans/HDDs/modelzoo/koanGAN/embers/quoxal-koancept-sylvaleonsce.pt",
        "/home/hans/HDDs/modelzoo/koanGAN/embers/koancept-finetune.pkl",
        blend_strategy="crossover",
        number=10,
        out_dir="/home/hans/modelzoo/koanGAN/embers/",
    )
    modelsoup(
        "/home/hans/HDDs/modelzoo/koanGAN/embers/quoxal-koancept-sylvaleonsce.pt",
        "/home/hans/HDDs/modelzoo/koanGAN/embers/quoxal-finetune.pkl",
        blend_strategy="crossover",
        number=10,
        out_dir="/home/hans/modelzoo/koanGAN/embers/",
    )
