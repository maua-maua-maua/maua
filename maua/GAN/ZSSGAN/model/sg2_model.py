...
from maua.GAN.ZSSGAN.model.op import FusedLeakyReLU, conv2d_gradfix, fused_leaky_relu, upfirdn2d
...
class Upsample(nn.Module):
    ...
    def forward(self, input):
        out = upfirdn2d.upfirdn2d(input, self.kernel, up=self.factor, down=1, pad=self.pad)
...
class Downsample(nn.Module):
    ...
    def forward(self, input):
        out = upfirdn2d.upfirdn2d(input, self.kernel, up=1, down=self.factor, pad=self.pad)
...
class Blur(nn.Module):
    ...
    def forward(self, input):
        out = upfirdn2d.upfirdn2d(input, self.kernel, pad=self.pad)
...
class EqualConv2d(nn.Module):
    ...
    def forward(self, input):
        out = conv2d_gradfix.conv2d(
            input, self.weight * self.scale, bias=self.bias, stride=self.stride, padding=self.padding
        )
...
class StyledConv(nn.Module):
    def __init__(
        self, in_channel, out_channel, kernel_size, style_dim, upsample=False, blur_kernel=[1, 3, 3, 1], demodulate=True
    ):
        super().__init__()
...
class Generator(nn.Module):
    def __init__(
        self, size, style_dim, n_mlp, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, constant_input=True
    ):
        super().__init__()
