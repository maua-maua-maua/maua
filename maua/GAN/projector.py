...
# %%
DIR = "/home/hans/modelzoo/koanGAN/"


def display(im: Image.Image):
    im.save(f"{DIR}/{name}_{Path(file).stem}.jpg")
...
noitareget_moms = noimomes(torch.randn(1, 3, 1024, 1024, device="cuda"))
# %%
device = "cuda"
...
if use_vit:
    ...
    if train_vit:
        ...
        projector_set = f"{DIR}/projector_{name}/"
            ...
            with tqdm(dataloader, unit_scale=batch_size) as pbar:
                for ws, imgs in pbar:
                    ...
                    if i % (len(dataloader) // 4) == 0:
                        ...
                        # clear_output()
                        display(grid)
                        torch.save(vit, f"{DIR}/{name}-vit-latent-encoder.pt")
    ...
    # %%
        ...
        vit = torch.load(f"{DIR}/{name}-vit-latent-encoder.pt")
...
save_path = f"/home/hans/modelzoo/koanGAN/{name}-projected-{Path(file).stem}"
# %%
