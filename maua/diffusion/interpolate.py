...
def interpolate_images(
    image_dir,
    prompt,
    output_file="output/interpolated.mp4",
    n_frames=120,
    fps=12,
    size=512,
    timesteps=100,
    ...
    images = list(reversed(sorted(glob(image_dir + "/*"))))
    ...
    with torch.autocast("cuda"), torch.inference_mode():
        ...
        if interp == "spline":
            ...
            interpolated = out.permute(2, 0, 1).reshape(n_frames, c, h, w)

        print("decoding latents...")
        with VideoWriter(output_file=output_file, output_size=(size, size), fps=fps) as video:
...
if __name__ == "__main__":
    interpolate_images(image_dir=sys.argv[1], prompt=sys.argv[2])
