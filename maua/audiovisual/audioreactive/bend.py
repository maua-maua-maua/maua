...
class NetworkBend(torch.nn.Module):
    """Base network bending class

    Args:
        sequential_fn (function): Function that takes a batch of modulation and creates torch.nn.Sequential
        modulation (torch.tensor): Modulation batch
    """
...
class Print(torch.nn.Module):
    ...
    def forward(self, x):
        print(x.shape, [x.min().item(), x.mean().item(), x.max().item()], torch.std(x).item())
        return x
...
class Translate(NetworkBend):
    ...
    Args:
        modulation (torch.tensor): [0.0-1.0]. Batch of modulation
    ...
    def __init__(self, modulation, h, w, noise):
        sequential_fn = lambda b: torch.nn.Sequential(
            torch.nn.ReflectionPad2d((int(w / 2), int(w / 2), 0, 0)),
            torch.nn.ReflectionPad2d((w, w, 0, 0)),
            torch.nn.ReflectionPad2d((w, 0, 0, 0)),
            AddNoise(noise),
            kT.Translate(b),
            kA.CenterCrop((h, w)),
        )
...
class Zoom(NetworkBend):
    ...
    def __init__(self, modulation, h, w):
        ...
        sequential_fn = lambda b: torch.nn.Sequential(
            torch.nn.ReflectionPad2d(padding), kT.Scale(b), kA.CenterCrop((h, w))
        )
...
class Rotate(NetworkBend):
    ...
    def __init__(self, modulation, h, w):
        ...
        sequential_fn = lambda b: torch.nn.Sequential(
            torch.nn.ReflectionPad2d(padding), kT.Rotate(b), kA.CenterCrop((h, w))
        )
